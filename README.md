...
2. **配置 NewTab**：启用 TabSyncer 新标签页，把常用 URL 或快照加入快捷入口。
3. **整理当前 Tab**：从 NewTab 进入 Tabout，整理当前已打开的标签页。
4. **保存快照**：把当前窗口、全部窗口或整理后的 Tab 保存为快照。
5. **管理快照**：在首页用列表、分组、搜索、合并、恢复等能力维护快照。
6. **查看关联图谱**：通过图谱理解快照之间的关系，找回相关上下文。
7. **继续工作**：从 NewTab 快捷入口或首页恢复快照，回到之前的工作现场。
